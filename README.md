## hi, i'm marc 👋

backend / full-stack developer focused on **java, spring boot, and scalable backend systems**

---

### 🛠 tech stack

- java (17/21), spring boot, spring security, jpa
- postgresql, mysql, mongodb, redis, supabase
- docker, docker compose, nginx, aws (ec2, s3, cloudwatch)
- spring kafka, kafka-compatible brokers (redpanda, dev)
- rest apis, microservices, event-driven patterns
- next.js, typescript, react

---

### 📦 projects

| project | link |
|---------|------|
| weekly — habits & strategy | [weekly.mfajardo.com](https://weekly.mfajardo.com) |
| focus — todo + time focus | [focus.mfajardo.com](https://focus.mfajardo.com) |
| spring boot starter | [github.com/mc44/spring-backend-starter](https://github.com/mc44/spring-backend-starter) |
| multi-tenant auth service | [github.com/mc44/auth-service](https://github.com/mc44/auth-service) |
| blog cms (microservices) | [github.com/mc44/blog-cms-microservices](https://github.com/mc44/blog-cms-microservices) · [blog.mfajardo.com](https://blog.mfajardo.com) *(planned)* |

---

### 📈 currently

- shipping focus + weekly on the same next.js stack
- dockerized auth and cms services on a vultr vps
- improving system design and backend depth

---

### 📫 reach me

- portfolio: [mfajardo.com](https://mfajardo.com)
- email: marcfajardo14@gmail.com
- linkedin: [marc fajardo](https://www.linkedin.com/in/marc-fajardo-1b9412222)
